...
func TestSuccessfulSignedGet(t *testing.T) {
	c := basicConsumer()
	m := newMocks(t)
	m.install(c)
	...
	resp, err := c.Get(
		"http://www.mrjon.es/someurl", map[string]string{"key": "val"}, nil)
...
func TestSuccessfulSignedPost(t *testing.T) {
	c := basicConsumer()
	m := newMocks(t)
	m.install(c)
	...
	resp, err := c.Post(
		"http://www.mrjon.es/someurl", "REQUEST_BODY", map[string]string{"key": "val"}, nil)
